...
#
# This component is responsible for rendering the KPI section at the top of the dashboard.
# It displays three KPI cards:
#   - Total Revenue (JPY)
#   - Total Units Sold
#   - Best-Selling Publisher
#
# How it works (for teammates):
# - layout() returns an empty <div> container; it does NOT create any KPI cards by itself.
# - The Dash callback `update_kpis` dynamically generates and injects the KPI cards
#   into this container based on the selected date range.
#
# - To add a new KPI card, you only need to:
#     1. Create a new helper function containing the SQL query required to compute
#        the new metric.
#     2. Add a new `dbc.Col(...)` block inside the `cards` layout in `update_kpis`
#        so the new metric appears as an additional card.
